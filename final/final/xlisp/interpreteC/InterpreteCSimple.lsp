...
(defun modificar_Valor_Var_En_Mem (nombre NuevoValor listaPares)
  (if (null listaPares)
    nil
    (if (equal nombre (caar listaPares) )
      (cons (list nombre NuevoValor ) (cdr listaPares) )
      (cons (car listaPares ) 
        (modificar_Valor_Var_En_Mem 
          nombre NuevoValor (cdr listaPares)
        )
      )
    )
  )
...
(defun agregar_Var_A_Mem (var mem)
  (if (null var) 
    ...
    (if (equal (length var) 1)
      ...
      (if (equal (nth 1 var) '= )
        (agregar_Var_Con_Asignacion (car var) (caddr var) 
          (agregar_Var_A_Mem (cdddr var) mem)
        )
  ...
  (if (null expr) 
    ...
    (if (esOperadorLisp (car expr))
      ...
      (if (atom (car expr))
        ...
        (inf_a_pref (cdr expr) ops 
            (cons (car (inf_a_pref (car expr))) vars)
        )
      )
    )  
  )
...
(defun armar_Expresion (sentencia mem)
  (if (null sentencia)
    ...
    (cond
      ...
      ( (not (atom (car sentencia))) 
        (cons (list (armar_Expresion (car sentencia) mem)) 
          (armar_Expresion (cdr sentencia) mem)) 
        )
      ;Si es variable, hay que buscar en memoria el valor
      ( (esVariable (car sentencia) mem) 
        (cons (getVarValue (car sentencia) mem) 
          (armar_Expresion (cdr sentencia) mem)) 
        )
      ;Si es un operador traducirlo a los de lisp
      ( (esOperadorEnC (car sentencia)) 
        (cons (traducirOp (car sentencia)) 
          (armar_Expresion (cdr sentencia) mem)) 
        )
...
(defun asignacion (expr memoria)
  (if (esVariable (car expr) memoria) 
    (cond
      ( (equal (nth 1 expr) '=) 
        (modificar_Valor_Var_En_Mem (car expr) 
          (evaluar (cddr expr) memoria)
          memoria
        )
      )
      ( (equal (nth 1 expr) '++) 
        (asignacion (list (car expr) '= (car expr) '+ 1 ) memoria)
      )
      ( (equal (nth 1 expr) '--) 
        (asignacion (list (car expr) '= (car expr) '- 1 ) memoria)
      )
      ( t (asignacion (list 
            (car l) '= (car l) (nth 1 expr) (nth 3 expr)) 
          memoria)
      )
    )
...
(defun procesar_if (prg mem entrada salida)
  (if (not (equal (evaluar (cadar prg) mem) 0)) 
    (ejecutar (append (list (nth 2 (car prg))) (cdr prg)) 
      mem entrada salida
    )
    (if (equal (length (car prg)) 5) 
      (ejecutar (append (list (nth 4 (car prg))) (cdr prg))
        mem entrada salida
      )
...
(defun procesar_scanf (prg mem entrada salida)
  (ejecutar (cdr prg) 
    (modificar_Valor_Var_En_Mem (cadar prg) (car entrada) mem)
    (cdr entrada) salida
  )
)

(defun procesar_printf (prg mem entrada salida)
  (ejecutar (cdr prg) mem entrada 
    (append salida (list (evaluar (cdar prg) mem))) 
  )
)
  ...
  (if (null prg) 
    ...
    (cond
      ...
      ( (esAsignacion (car prg) mem)
        (ejecutar (cdr prg) (asignacion (car prg) mem) entrada salida) )
      ( (esFuncion prg 'if) (procesar_if prg mem entrada salida) )
      ( (esFuncion prg 'while) (procesar_while prg mem entrada salida) )
      ( (esFuncion prg 'repeat_aux) 
        (procesar_repeat_aux prg mem entrada salida) )
...
(defun run (prg &optional (entrada nil) (mem nil) )
  (if (null prg)
    nil
    (cond
      ( (isVarDef prg) (run (cdr prg) entrada 
        (agregar_Var_A_Mem (cdar prg) mem)) 
      )
